...
{
    /**
     * Test that the BadgeUnlocked event is dispatched with the correct user and badge name
     */
    ...
    public function test_dispatches_a_badge_unlocked_listener_when_a_comment_is_written(): void
    {
        ...
        Comment::factory()->count(8)->for($user)->create();
        ...
        Event::assertDispatched(BadgeUnclocked::class, function ($event) use ($user) {
            return $event->badge['user']->id === $user->id && $event->badge['badge_name'] === 'Beginner';
        });
    }
